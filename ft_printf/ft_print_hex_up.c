					...
					unsigned int arg, int len)
{
	if (info->accur > len && info->width >= info->accur)
	{
		...
		ft_print_zeros_hex_u(info->accur - len, arg, 0, info);
	else if (info->width < 0 && info->accur <= len)
		ft_print_spaces_hex_u(ft_mod(info->width) - len, arg, 1, info);
	else if (info->width < 0 && info->accur > len)
	{
		ft_put_zero(info->accur - len, info);
		ft_putnbr_base(arg, info, 65);
					...
					unsigned int arg, int len)
{
	if (info->width > 0 && info->accur <= len && info->width > len)
	{
		if (info->is_zero == 1 && info->accur < 0)
			ft_print_zeros_hex_u(info->width - len, arg, 0, info);
		else
			ft_print_spaces_hex_u(info->width - len, arg, 0, info);
		return ;
	}
	else
...
static void			ft_print_zero_hex_n(t_struct *info,
					unsigned int arg)
{
	if (info->width < 0 && info->accur == 0)
		ft_put_space(ft_mod(info->width), info);
	else if (info->width < 0 && info->accur <= 1)
		ft_print_spaces_hex_u(ft_mod(info->width) - 1, arg, 1, info);
	else if (info->accur > 1 && info->width >= info->accur)
	{
		...
		ft_print_zeros_hex_u(info->accur - 1, ft_mod(arg), 0, info);
	else if (info->width < 0 && info->accur > 1)
	{
		ft_print_zeros_hex_u(info->accur - 1, arg, 0, info);
		ft_put_space(ft_mod(info->width) - info->accur, info);
	}
...
static void			ft_print_zero_hex(t_struct *info, unsigned int arg)
{
	...
	if (info->width > 0 && info->accur <= 1)
	{
		if (info->is_zero && (info->accur <= 0))
			ft_print_zeros_hex_u(info->width - 1, arg, 0, info);
		else if (info->accur)
			ft_print_spaces_hex_u(info->width - 1, arg, 0, info);
		else
